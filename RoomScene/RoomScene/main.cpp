...
GLint mirror_projection_matrix_loc;
GLint mirror_view_matrix_loc;
GLint mirror_model_matrix_loc;
...
void init()
{
	...
	shader_program = CreateAndLinkProgram("shadowMapVertexShader.glsl", "shadowMapFragmentShader.glsl",
		position_loc, "position", normal_loc, "normal", tex_coord_loc, "tex_coord");
	mirror_program = CreateAndLinkProgram("mirror_vertex.glsl", "mirror_fragment.glsl",
		position_loc, "position", normal_loc, "normal", tex_coord_loc, "tex_coord");
	...
	GLint mirrorTex = glGetUniformLocation(cube_program, "mirror_tex");

	glUniform1i(cube_wood_tex_loc, 0);
	glUniform1i(shadowTex, 1);
	glUniform1i(shadowTex2, 2);
	glUniform1i(mirrorTex, 3);
	...
	CreateFramebufferForMirror(&mirrorFramebuffer, glm::vec2(win_width, win_height));
	mirror_view_matrix_loc = glGetUniformLocation(shader_program, "view_matrix");
	mirror_projection_matrix_loc = glGetUniformLocation(shader_program, "projection_matrix");
	mirror_model_matrix_loc = glGetUniformLocation(shader_program, "model_matrix");
	...
	my_lavaLamp = LoadOBJ("lamp2.obj", position_loc, normal_loc, tex_coord_loc);
...
void render()
{
	...
	model_matrix = glm::translate(glm::mat4(1.0f), glm::vec3(-4.5, -2, -2.14)) * scaleMatrix *glm::scale(glm::mat4(),glm::vec3(2.f,7.f,2.f)) * glm::rotate(glm::mat4(1.0f), glm::radians(90.f), glm::vec3(0.f, 1.f, 0.f));
	tempObject = new graphicsObject(my_lamp, model_matrix, cube_model_matrix_loc, clock_white_tex);
	objects.push_back(tempObject);
	...
	glUniformMatrix4fv(shader_model_matrix_loc, 1, GL_FALSE, glm::value_ptr(model_matrix));
	...
	glUseProgram(shader_program);
	glBindFramebuffer(GL_FRAMEBUFFER, mirrorFramebuffer.framebuffer);
	glActiveTexture(GL_TEXTURE0);
		
	glUniformMatrix4fv(mirror_view_matrix_loc, 1, GL_FALSE, glm::value_ptr(CameraData.view_matrix));
	glUniformMatrix4fv(mirror_projection_matrix_loc, 1, GL_FALSE, glm::value_ptr(CameraData.projection_matrix));
	glUniformMatrix4fv(mirror_model_matrix_loc, 1, GL_FALSE, glm::value_ptr(model_matrix2));*/
	//DrawObjects(objects);
	...
	glBindTexture(GL_TEXTURE_2D, mirrorFramebuffer.mirrorTexture);
