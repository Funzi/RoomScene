...
namespace PV112
{
	...
	void CreateFramebufferForMirror(MirrorFramebuffer *framebuffer, glm::vec2 screenSize)
	{
		glGenFramebuffers(1, &framebuffer->framebuffer);
		glBindFramebuffer(GL_FRAMEBUFFER, framebuffer->framebuffer);

		glGenTextures(1, &framebuffer->mirrorTexture);
		glBindTexture(GL_TEXTURE_2D, framebuffer->mirrorTexture);
		//glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, (GLsizei)screenSize.x, (GLsizei)screenSize.y, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);


		glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, (GLsizei)4096, (GLsizei)4096, 0, GL_3D_COLOR_TEXTURE, GL_FLOAT, NULL);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
		...
		glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR, framebuffer->mirrorTexture, 0);
		GLenum Status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
		if (Status != GL_FRAMEBUFFER_COMPLETE) {
